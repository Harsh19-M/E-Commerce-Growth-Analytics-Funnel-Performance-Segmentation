...
- **Python Funnel Analysis, Segmentation & Feature Engineering** Extended the SQL output in Python by constructing a behaviorally ordered full-funnel framework (Visit → View → ATC → Purchase) and engineering behavioral features (conversion, lift, engagement, intent). Applied explicit rule-based segmentation using conditional logic to classify users by Engagement Level and Purchase Intent, ensuring explainable and transparent segmentation. Created drop-off tables while validating experiment balance, metric consistency, and exposure integrity.

- **EDA, Experiment Insights & Data Re-Loading** Identified Variant B as the top performer (5 of 8 experiments, up to +211% lift) and showed that completed purchase—not CTR or ATC—was the correct decision metric. Mapped performance gains to specific UI elements (Trust Badges, Urgency Banners) and reloaded enriched tables into PostgreSQL for BI use and reuse.

- **Statistical Testing & Validation** Applied one-sided proportion z-tests, chi-square independence tests, ANOVA, and logistic regression models to validate experiment results, segment effects, and UI performance. Controlled for confounding factors and confirmed Variant B as the consistently significant conversion driver.
