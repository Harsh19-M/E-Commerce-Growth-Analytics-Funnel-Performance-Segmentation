...
- **Tableau + Reporting:** Designed analysis-driven Tableau dashboards using Python-engineered metrics to explain funnel drop-offs, experiment lift, and behavioral segmentation, with user-level drill-downs (via anonymized User IDs) enabling stakeholders to trace segment insights back to individual user behavior and outcomes.

- **PostgreSQL Data Aggregation & Modeling:** Aggregated 80K sessions, 8K users, 30K orders, and 8 experiments into a 1,563-row analysis-ready table using multi-step CTEs. Validated experiment exposure, session logic, and event tracking with window functions, filtered counts, distinct aggregations, and null handling to produce a clean, trusted dataset for downstream analysis.
- **Python Funnel Analysis, Segmentation & Feature Engineering:** Built a full-funnel framework (Visit → View → ATC → Purchase) and engineered 7 behavioral features (conversion, lift, engagement, intent). Created explainable, rule-based user segments and drop-off tables while validating experiment balance, metric consistency, and exposure integrity.
- **Experiment Insights & Data Re-Loading:** Identified Variant B as the top performer (5/8 experiments, up to +211% lift) and showed that completed purchase—not CTR or ATC—was the correct decision metric. Mapped lift to specific UI elements (Trust Badges, Urgency Banners) and reloaded enriched tables into PostgreSQL for BI and reuse.
- **Statistical Testing & Validation:** Applied z-tests, chi-square tests, ANOVA, and logistic regression to validate experiment results, segment effects, and UI performance. Controlled for confounders and confirmed Variant B as the consistently significant conversion driver.
...
- **End-to-End Analytics Pipeline:** Aggregated 80K+ sessions, 8K users, 30K orders, and 8 experiments into a 1,563-row analysis-ready table using PostgreSQL (CTEs, window functions, filtered counts), then extended analysis in Python and delivered BI-ready outputs.
- **Funnel Analysis, Segmentation & Feature Engineering:** Built a full conversion funnel (Visit → View → ATC → Purchase), engineered behavioral features (conversion, lift, engagement, intent), and created explainable user segments for experiment evaluation.
- **Experimentation & Statistical Validation:** Identified Variant B as the top performer (up to +211% lift) and validated results using z-tests, chi-square tests, ANOVA, and logistic regression to ensure statistically sound decisions.
- **Business Impact & BI Delivery:** Translated analysis into scalable growth recommendations and delivered Tableau dashboards with segment-level and user-level drill-downs to support stakeholder decisions.
